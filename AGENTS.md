...
- In `apps/web` `PmMarkdownEditor`, StarterKit inline-code input behavior can be inconsistent in the full React runtime; adding an explicit `editorProps.handleTextInput` backtick fallback avoids raw literal backticks and reliably applies the `code` mark.
- In the `PmMarkdownEditor` backtick fallback, replace the range `[openingPos, from]` (not `from + 1`) and clear `storedMarks` for `code`; otherwise the fallback can delete the next character and keep subsequent typing inside inline code.
- In `packages/editor-pm` markdown shortcut rules, the link regex must exclude image prefixes (`(?<!!)`) or `![alt](url)` typing is partially converted into a link with a literal leading `!`.
...
- In `packages/editor-pm` delimiter rollover, auto-converting list lines to markdown-source paragraphs can destabilize caret/Enter behavior and ordered-list numbering; keep list editing in rendered mode and reserve source projection for blockquotes/top-level blocks.
